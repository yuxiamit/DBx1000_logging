...
RC 
row_t::init(table_t * host_table, uint64_t part_id, uint64_t row_id, void * mem) {
	_row_id = row_id;
	_part_id = part_id;
...
void row_t::set_value(int col_id, SInt32 value) {
	// convert to int64
	uint64_t tval = value;
	set_value(col_id, &tval);
}

void row_t::set_value(int col_id, UInt32 value) {
	// convert to int64
	uint64_t tval = value;
	set_value(col_id, &tval);
}
