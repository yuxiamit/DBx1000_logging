...
void LoggingThread::init()
{
	...
	// initializer for multi-SPSC queues
			...
			for(uint32_t i=0; i<num_worker; i++)
			{
				...
				{
					SPSCPools[i][j] = (poolItem*) _mm_malloc(sizeof(poolItem) * g_poolsize_wait + ALIGN_SIZE, ALIGN_SIZE);
			...
			SPSCPoolStart = (volatile uint64_t *) _mm_malloc(sizeof(uint64_t) * POOL_SE_SPACE * num_worker, ALIGN_SIZE);
			SPSCPoolEnd = (volatile uint64_t *) _mm_malloc(sizeof(uint64_t) * POOL_SE_SPACE * num_worker, ALIGN_SIZE);
			for(uint32_t i=0; i<num_worker; i++)
			{
				SPSCPoolStart[i * POOL_SE_SPACE] = 0;
				SPSCPoolEnd[i * POOL_SE_SPACE] = 0;
			}
			workerDone = (volatile uint64_t *) _mm_malloc(sizeof(uint64_t) * POOL_SE_SPACE, ALIGN_SIZE);
			workerDone[0] = 0;
#endif
		}
	#endif
...
LoggingThread::run()
{
  ...
  //#if LOG_ALGORITHM == LOG_TAURUS || LOG_ALGORITHM == LOG_BATCH
  	set_affinity(logger_id * NUM_CORES_PER_SLOT); // first CPU per socket
	//set_affinity(logger_id); // first CPU per socket
	printf("Setting loggr %lu to CPU node %d\n", GET_THD_ID, logger_id * NUM_CORES_PER_SLOT);
  #endif
...
// if compression is not on, entry must be 0xbeef.					
					...
					if(entry[0] == 0x7f)
					{
						for(uint32_t workerId=0; workerId<num_worker; workerId += POOL_SE_SPACE)
						{
							...
							poolItem * pi = SPSCPools[workerId/POOL_SE_SPACE][SPSCPoolEnd[workerId] % g_poolsize_wait]; //(poolItem*) _mm_malloc(sizeof(poolItem), ALIGN_SIZE);
							pi->oldp = entry;
							pi->rasterized = 0;
...
#endif
				{
					...
					{
						poolItem * pi = SPSCPools[workerId / POOL_SE_SPACE][SPSCPoolStart[workerId] % g_poolsize_wait]; //(poolItem*) _mm_malloc(sizeof(poolItem), ALIGN_SIZE);
						if(pi->LSN[0] - 1 < rlv)
							rlv = pi->LSN[0] - 1;
					}
