...
/*#if CC_ALG == SILO
        ...
        lsn_vec = (lsnType*) _mm_malloc(sizeof(lsnType) * 8, ALIGN_SIZE); 
        readLV = lsn_vec + 4; // (lsnType*) _mm_malloc(sizeof(lsnType) * 4, ALIGN_SIZE); 
#else
        if (cache == NULL)
        {
            lsn_vec = (lsnType*) _mm_malloc(sizeof(lsnType) * g_num_logger * 2, ALIGN_SIZE); 
...
// for SILO we already done the memory update in silo_validate
        ...
        {
//            assert((uint64_t)row->table > 0x7ff000000000); // valid row or not?
...
//#endif
            ...
            // otherwise if full
            LockTableListItem *lti = (LockTableListItem*) _mm_malloc(sizeof(LockTableListItem), ALIGN_SIZE);
    ...
    private:
        ...
        LockTable() { // assuming single thread
            ...
            uint32_t ltiSize = sizeof(LockTableListItem) / ALIGN_SIZE * ALIGN_SIZE + ALIGN_SIZE;
            cout << "ltiSize=" << ltiSize << endl;
            //uint32_t ltiCacheSize = sizeof(lsnType) * g_num_logger * 4 / ALIGN_SIZE * ALIGN_SIZE + ALIGN_SIZE;
            char * ltiBuffer = (char *) _mm_malloc(ltiSize * locktable_size * g_locktable_init_slots, ALIGN_SIZE);
            //char * ltiCache = (char *) _mm_malloc(ltiCacheSize * locktable_size * g_locktable_init_slots, ALIGN_SIZE);
