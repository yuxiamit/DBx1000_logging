...
void Row_mvcc::init(row_t * row) {
	...
	_write_history = (WriteHisEntry *) _mm_malloc(sizeof(WriteHisEntry) * _his_len, ALIGN_SIZE);
	_requests = (ReqEntry *) _mm_malloc(sizeof(ReqEntry) * _req_len, ALIGN_SIZE);
	...
	blatch = false;
	latch = (pthread_mutex_t *) _mm_malloc(sizeof(pthread_mutex_t), ALIGN_SIZE);
...
{
	if (list == 0) {
		WriteHisEntry * temp = (WriteHisEntry *) _mm_malloc(sizeof(WriteHisEntry) * _his_len * 2, ALIGN_SIZE);
		for (uint32_t i = 0; i < _his_len; i++) {
		...
		assert(list == 1);
		ReqEntry * temp = (ReqEntry *) _mm_malloc(sizeof(ReqEntry) * _req_len * 2, ALIGN_SIZE);
		for (uint32_t i = 0; i < _req_len; i++) {
...
Row_mvcc::reserveRow(ts_t ts, txn_man * txn)
{
	...
	if (!_write_history[idx].row) {
		_write_history[idx].row = (row_t *) _mm_malloc(sizeof(row_t), ALIGN_SIZE);
