...
if workload == 'TPCC' and log_type == 'D':
  log_buffer *= 10
app = 'rundb_NO_%s' % workload # "rundb_T%s_%s" % (log_type, workload)
...
whs = [1, 2, 5, 10, 15, 20, 25, 30, 35, 40, 45, 50, 55, 60, 64]
    ...
    for wh in whs:
      ...
      while True:
          try:
            ...
            ret = subprocess.check_output("numactl -i all -- ./%s -Ln%d -t%d -Lb%d -n%d" % (app, log_num, t, log_buffer, wh), shell=True).decode('ascii')
            # ret = subprocess.check_output("./%s -Ln%d -t%d -Lb524288000" % (app, log_num, t), shell=True).decode('ascii') # to get the core dump
            # print(ret)
            open(resDir + '/%s_ln%d_th%d_%d_n%d.txt' % (app, log_num, t, k, wh), 'w').write(ret)
            # print ret
